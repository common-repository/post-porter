<?php
/**
 * Unit tests for POST_PORTER_Background_Process.
 *
...
/**
 * Class POST_PORTER_Test_Background_Process
 */
class POST_PORTER_Test_Background_Process extends WP_UnitTestCase {
	/**
	 * Instance of POST_PORTER_Background_Process
	 *
	 * @var MockObject|POST_PORTER_Background_Process|(POST_PORTER_Background_Process&MockObject)
	 */
	private $wpbp;
	...
	public function setUp(): void {
		...
		$this->wpbp = $this->getMockForAbstractClass( POST_PORTER_Background_Process::class );
	...
	private function getWPBPProperty( string $name ) {
		try {
			$property = new ReflectionProperty( 'POST_PORTER_Background_Process', $name );
		} catch ( Exception $e ) {
	...
	private function executeWPBPMethod( string $name, ...$args ) {
		try {
			$method = new ReflectionMethod( 'POST_PORTER_Background_Process', $name );
	...
	public function test_push_to_queue() {
		$this->assertClassHasAttribute( 'data', 'POST_PORTER_Background_Process', 'class has data property' );
	...
	public function test_maybe_handle_cancelled() {
		...
		$this->wpbp->save();
		$this->assertCount( 2, $this->wpbp->get_batches() );
		update_site_option( $this->executeWPBPMethod( 'get_status_key' ), POST_PORTER_Background_Process::STATUS_CANCELLED );
